...
User decryption in LuxFHE enables the secure sharing or reuse of encrypted data under a new public key without exposing the plaintext.
...
> **Warning:** For the user to be able to user decrypt (also called re-encrypt) the ciphertext value the access control (ACL) needs to be set properly using the `FHE.allow(ciphertext, address)` function in the solidity contract holding the ciphertext.
> For more details on the topic please refer to [the ACL documentation](https://docs.lux.network/fhe/acl).
